...
**Models benchmarked so far (all Path 3, self-hosted on vLLM, `g6e.12xlarge` / 4x L40S):** Qwen3.6-35B-A3B, Qwen3-Coder-30B-A3B-Instruct, and Qwen3-Coder-Next. **Coming soon:** Path 1 (Anthropic family on Bedrock -- Claude Opus/Sonnet/Haiku) and Path 2 (open-weight on Bedrock via the LiteLLM proxy -- Kimi, DeepSeek, Mistral, GLM, MiniMax, …).
...
### Results -- 5 tasks x 3 self-hosted models

All cells are task scores (0-100), the mean of the 4 artifact totals per (task x model). All three models were **self-hosted via vLLM on a single `g6e.12xlarge` (4x L40S)** and scored by the same judge (`codex exec`, `gpt-5.6-sol`, high reasoning effort). Bold = top score in row. Other hosting paths are **coming soon**.

| Task | Difficulty | Qwen3.6-35B | Qwen3-Coder-30B | Qwen3-Coder-Next³ |
|------|-----------|------------:|----------------:|:-----------------:|
| `remove-faiss` | Medium | **59.25** | 49.0 | n/a |
| `remove-efs-from-terraform-aws-ecs` | Medium | **63.0** | 45.0 | n/a |
| `ssrf-hardening-outbound-url-validation` | Medium | **55.75** | 0.0 ⁴ | n/a |
| `migrate-ecs-env-vars-to-secrets-manager` | High | **54.5** | 43.5 | n/a |
| `replace-keycloak-db-password-with-rds-iam` | High | **48.75** | 33.25 | n/a |
| **Mean** | | **56.25** | **34.15** | **n/a** |

³ Qwen3-Coder-Next (79.6B, ~160 GB weights) **could not be benchmarked on this node.** On a `g6e.12xlarge` the weights leave room for only a ~16K context window, but agentic coding tasks need 100K-250K input tokens per request, so every task overflows the window on the first prompt. It needs a larger-VRAM node (e.g. `g6e.48xlarge`) to serve a >=200K window. The `/benchmark` skill now enforces a 200K-minimum gate and refuses to run below it. See [self-hosted/vllm/models/qwen3-coder-next.md](self-hosted/vllm/models/qwen3-coder-next.md).

⁴ **Genuine model failure, scored 0.** Qwen3-Coder-30B misread this design task as an implementation task -- on both attempts it spent all 60 turns editing repo source files and never wrote any of the four required design artifacts. The judge records a missing-artifact folder as a 0 with a `MODEL FAILURE` verdict rather than dropping it. Excluding this task, Qwen3-Coder-30B averages 42.7.

### Per-model leaderboard (self-hosted, so far)

| Rank | Model | Params (active) | Avg score | # tasks |
|-----:|-------|----------------|----------:|--------:|
| 1 | Qwen3.6-35B-A3B | 35.9B (3B) | **56.25** | 5 |
| 2 | Qwen3-Coder-30B-A3B-Instruct | 30.5B (3B) | **34.15** | 5 |
| - | Qwen3-Coder-Next | 79.6B (3B) | not viable on this node | 0 |

**Coming soon:** Claude Opus/Sonnet/Haiku (Path 1, Bedrock) and the open-weight Bedrock models via the LiteLLM proxy (Path 2 -- Kimi, DeepSeek, Mistral, GLM, MiniMax, …).

### What the data says (so far)

These are early numbers from three self-hosted Qwen models on one GPU node; treat them as a starting point, not a final ranking. Cross-path comparisons wait until the Bedrock paths are run.

- **Qwen3.6-35B clearly leads the two that ran**, winning every task and averaging 56.25 vs 34.15 for Qwen3-Coder-30B. On this judge's calibration (median artifact ~60-70) that puts the 3.6 model at roughly median design quality and the coder-30B below it.
- **The judge is strict, and these are open-weight models on design (not coding) tasks.** Scores in the 45-63 range reflect artifacts that are serviceable but light on the specificity and risk-analysis the rubric rewards; this is expected for smaller models asked to produce design documentation rather than code.
- **Task type matters more than the difficulty label.** Both models did best on the bounded `remove-*` cleanups and worst on the multi-service AWS infrastructure tasks (`keycloak-rds-iam` was the low for both), where the design requires coordinating Terraform, IAM, and service wiring.
- **MoE economics are the reason to self-host these.** All three are 3B-active MoE models, so per-token compute (and cost) tracks the 3B active count, not the total -- the regime where a fixed-cost GPU node can beat per-token API pricing under load.
